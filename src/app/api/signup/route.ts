...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = await validateRequestBody(userSignupSchema, request);

    if (!validation.success) {
      return createValidationErrorResponse(validation.errors!);
    }

    const { email, password, full_name, phone, ...rest } = validation.data!;

    const role = "customer"; // Rol por defecto
