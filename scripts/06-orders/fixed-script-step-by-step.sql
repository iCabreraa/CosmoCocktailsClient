-- Script corregido para evitar el error de restricción
-- Ejecutar este SQL paso a paso en el SQL Editor del dashboard de Supabase

-- PASO 1: Primero actualizar todos los pedidos existentes con estado "paid" a "ordered"
UPDATE public.orders
SET status = 'ordered'
WHERE status = 'paid';

-- PASO 2: Verificar que no quedan pedidos con estado "paid"
SELECT status, COUNT(*) as count
FROM public.orders 
WHERE status = 'paid'
GROUP BY status;

-- PASO 3: Eliminar la restricción existente
ALTER TABLE public.orders 
DROP CONSTRAINT IF EXISTS orders_status_check;

-- PASO 4: Crear nueva restricción con los estados válidos
ALTER TABLE public.orders 
ADD CONSTRAINT orders_status_check 
CHECK (status IN ('ordered', 'preparing', 'on_the_way', 'completed', 'pending', 'cancelled'));

-- PASO 5: Actualizar el RPC para crear pedidos directamente con estado "ordered"
