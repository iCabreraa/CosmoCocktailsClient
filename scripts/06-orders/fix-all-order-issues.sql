-- Script final para corregir todos los problemas de estados y restricciones
...
-- 2. Eliminar la restricción de estado existente
ALTER TABLE public.orders 
DROP CONSTRAINT IF EXISTS orders_status_check;

-- 3. Crear nueva restricción con todos los estados válidos
ALTER TABLE public.orders 
ADD CONSTRAINT orders_status_check 
CHECK (status IN ('paid', 'ordered', 'preparing', 'on_the_way', 'completed', 'pending', 'cancelled'));

-- 4. Actualizar pedidos con estado 'Pendiente' a 'ordered'
...
-- 7. Verificar estados de pedidos existentes
SELECT status, COUNT(*) as count
FROM public.orders 
GROUP BY status
ORDER BY status;

-- 8. Probar insertar un pedido con estado 'paid'
...
RETURNING id, status, order_ref;

-- 9. Limpiar el registro de prueba
